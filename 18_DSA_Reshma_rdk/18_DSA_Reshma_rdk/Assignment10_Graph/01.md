...
public:
    ...
    void acceptGraph_rdk() {
        cout << "\nEnter Adjacency Matrix (enter 0 if no edge):\n";
        for (int i = 0; i < nodes_rdk; i++) {
            for (int j = 0; j < nodes_rdk; j++) {
                cin >> adjMat_rdk[i][j];
                if (i < j && adjMat_rdk[i][j] != 0) {
                    Edge e;
                    e.src_rdk = i;
                    e.dest_rdk = j;
                    e.wt_rdk = adjMat_rdk[i][j];
...
int main() {
    ...
    while (true) {
        ...
        switch (choice_rdk) {
            case 1: g_rdk.acceptGraph_rdk(); break;
            case 2: g_rdk.kruskal_rdk(); break;
            case 3: 
                cout << "Exiting...\n";
                return 0;
            default: 
